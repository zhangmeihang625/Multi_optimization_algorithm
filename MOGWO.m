function [POS,POS_fit]=MOGWO(params,MultiObj)
...
for it=1:MaxIt
    ...
    disp(['MOGWO Iteration ' num2str(it) ': Number of solutions in the archive = ' num2str(numel(Archive))]);

    
    % Results
    POS_fit=GetCosts(Archive)';
    for  i=1:size(Archive,1)
    POS(i,:)=Archive(i).Position;
    end
  h_fig = figure(1);
 if(numOfObj==2)
    figure(h_fig); 
